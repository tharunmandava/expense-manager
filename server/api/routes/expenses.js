...
router.get("/total-of-group/:id", async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query("BEGIN");
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Expense created successfully", id: expense_id });
  } catch (error) {
    await pool.query("ROLLBACK");
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (
      participantAmounts &&
      typeof participantAmounts === "object" &&
      Object.keys(participantAmounts).length > 0
    ) {
      ...
      const participantQueries = Object.entries(participantAmounts).map(
        ([user_id, amount]) =>
